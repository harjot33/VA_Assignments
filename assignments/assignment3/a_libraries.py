...
def matplotlib_table(matrix: np.array) -> Tuple:
    ...
    Return the fig and ax as was shown in matplotlib_line_example.
    """
    collabels = list(range(matrix.shape[1]))
    fig, ax = plt.subplots()
    ax.set_axis_off()
    table = ax.table(
    cellText = matrix,
...
def plotly_polar_chart(df: pd.DataFrame):
    ...
    fig = px.scatter_polar(df_copy, r="x", theta="angle")
    return fig
...
def plotly_subgraphs(df: pd.DataFrame):
    ...
    """
    fig = make_subplots(start_cell="top-left",rows=2, cols=2)
    ...
    trace1 = go.Scatter(x = df['x3'], y = df['y3'], stackgroup='default')
    trace2 = go.Scatter(x = df['x4'], y = df['y4'], stackgroup='default')
